...
@dataclass
class BatchedEnergyTuckerSolveCache:
    basis: torch.Tensor
    k_base: torch.Tensor
    k_t: torch.Tensor
    gram: torch.Tensor
...
def _energy_tt_core0_grad(free_full_grad: torch.Tensor, core1: torch.Tensor, core2: torch.Tensor) -> torch.Tensor:
    q = int(free_full_grad.shape[0])
    right_rank = int(core2.shape[0])
    tmp = free_full_grad.reshape(q * q, q).matmul(core2.T).reshape(q, q * right_rank)
    return tmp.matmul(core1.reshape(core1.shape[0], q * right_rank).T)


def _energy_tt_core1_grad(free_full_grad: torch.Tensor, core0: torch.Tensor, core2: torch.Tensor) -> torch.Tensor:
    tmp = torch.matmul(core0.T.unsqueeze(0), free_full_grad.permute(1, 0, 2))
    return tmp.matmul(core2.T).permute(1, 0, 2).contiguous()


def _energy_tt_core2_grad(free_full_grad: torch.Tensor, core0: torch.Tensor, core1: torch.Tensor) -> torch.Tensor:
    q = int(free_full_grad.shape[0])
    left_rank = int(core0.shape[1])
    tmp = core0.T.matmul(free_full_grad.reshape(q, q * q)).reshape(left_rank, q, q)
    return tmp.permute(2, 0, 1).reshape(q, left_rank * q).matmul(core1.reshape(left_rank * q, core1.shape[2])).T


def _energy_tt_core0_grad_batched(free_full_grad: torch.Tensor, core1: torch.Tensor, core2: torch.Tensor) -> torch.Tensor:
    direction_count, q, _, _ = free_full_grad.shape
    right_rank = int(core2.shape[0])
    tmp = free_full_grad.reshape(direction_count, q * q, q).matmul(core2.T).reshape(direction_count, q, q * right_rank)
    return tmp.matmul(core1.reshape(core1.shape[0], q * right_rank).T)


def _energy_tt_core1_grad_batched(free_full_grad: torch.Tensor, core0: torch.Tensor, core2: torch.Tensor) -> torch.Tensor:
    tmp = torch.matmul(core0.T.unsqueeze(0).unsqueeze(0), free_full_grad.permute(0, 2, 1, 3))
    return tmp.matmul(core2.T).permute(0, 2, 1, 3).contiguous()


def _energy_tt_core2_grad_batched(free_full_grad: torch.Tensor, core0: torch.Tensor, core1: torch.Tensor) -> torch.Tensor:
    direction_count, q, _, _ = free_full_grad.shape
    left_rank = int(core0.shape[1])
    tmp = core0.T.unsqueeze(0).matmul(free_full_grad.reshape(direction_count, q, q * q)).reshape(direction_count, left_rank, q, q)
    return tmp.permute(0, 3, 1, 2).reshape(direction_count, q, left_rank * q).matmul(
        core1.reshape(left_rank * q, core1.shape[2])
    ).transpose(1, 2)
...
class EnergyTuckerTTGradientAccumulator:
    ...
    def flush_to(self, param_grads: dict[str, torch.Tensor]) -> None:
        ...
        _add_param_grad(param_grads, "quadratic.core0", _energy_tt_core0_grad(free_full_grad, core1, core2))
        _add_param_grad(param_grads, "quadratic.core1", _energy_tt_core1_grad(free_full_grad, core0, core2))
        _add_param_grad(param_grads, "quadratic.core2", _energy_tt_core2_grad(free_full_grad, core0, core1))
...
class BatchedEnergyTuckerTTGradientAccumulator:
    ...
    def flush_to(self, param_grads: dict[str, torch.Tensor]) -> None:
        ...
        core1 = quad.core1.to(device=grad_c.device, dtype=grad_c.dtype)
        core2 = quad.core2.to(device=grad_c.device, dtype=grad_c.dtype)
        _add_param_grad(param_grads, "quadratic.core0", _energy_tt_core0_grad_batched(free_full_grad, core1, core2))
        _add_param_grad(param_grads, "quadratic.core1", _energy_tt_core1_grad_batched(free_full_grad, core0, core2))
        _add_param_grad(param_grads, "quadratic.core2", _energy_tt_core2_grad_batched(free_full_grad, core0, core1))
...
def _dense_frozen_vjp(
    ...
    *,
    ...
    out_dim, lag_dim, x_dim = c.shape
    right_mats = x.matmul(c.reshape(out_dim * lag_dim, x_dim).T).reshape(x.shape[0], out_dim, lag_dim)
    ell_grad = torch.bmm(cot.unsqueeze(1), right_mats).squeeze(1)
    x_grad = None
    if include_x_grad:
        left_mats = ell.matmul(c.permute(0, 2, 1).reshape(out_dim * x_dim, lag_dim).T).reshape(ell.shape[0], out_dim, x_dim)
        x_grad = torch.bmm(cot.unsqueeze(1), left_mats).squeeze(1)
    ell_x_outer = torch.bmm(ell.unsqueeze(2), x.unsqueeze(1)).reshape(ell.shape[0], lag_dim * x_dim)
    _add_param_grad(param_grads, "quadratic.tensor", cot.T.matmul(ell_x_outer).reshape(out_dim, lag_dim, x_dim))
    return ell_grad, x_grad
...
def _energy_dense_frozen_vjp(
    ...
    *,
    ...
    if include_x_grad:
        ...
        x_grad = torch.bmm(cot.unsqueeze(1), left_mats).squeeze(1)
    ell_x_outer = torch.bmm(ell.unsqueeze(2), x.unsqueeze(1)).reshape(ell.shape[0], lag_dim * x_dim)
    grad_c = cot.T.matmul(ell_x_outer).reshape(out_dim, lag_dim, x_dim)
...
def _energy_tuckertt_frozen_vjp(
    ...
    *,
    ...
    z = x @ p.T
    q = int(c.shape[0])
    reduced_matrix = z_lag.matmul(c.permute(0, 2, 1).reshape(q * q, q).T).reshape(z_lag.shape[0], q, q)
    reduced_out = torch.bmm(reduced_matrix, z.unsqueeze(-1)).squeeze(-1)
    omega = cot @ p.T

    z_outer = torch.bmm(z_lag.unsqueeze(2), z.unsqueeze(1)).reshape(z_lag.shape[0], q * q)
    grad_c = omega.T.matmul(z_outer).reshape(q, q, q)
    right_mats = z.matmul(c.reshape(q * q, q).T).reshape(z.shape[0], q, q)
    grad_z_lag = torch.bmm(omega.unsqueeze(1), right_mats).squeeze(1)
    grad_z = torch.bmm(omega.unsqueeze(1), reduced_matrix).squeeze(1)
...
def _exact_frozen_solve_batched(
    ...
    *,
    ...
    solve_cache: BatchedEnergyTuckerSolveCache | None = None,
) -> torch.Tensor:
    ...
    if isinstance(dynamics.quadratic, EnergyTuckerTTQuadratic):
        ...
        if (
            ...
            and solve_cache.tau == float(tau)
            and solve_cache.transpose == bool(transpose)
            and solve_cache.basis.device == rhs.device
            and solve_cache.basis.dtype == rhs.dtype
        ):
            p = solve_cache.basis
            k_base = solve_cache.k_base
            k = solve_cache.k_t
            gram = solve_cache.gram
            eye = solve_cache.eye
        else:
            p = quad.basis.to(device=rhs.device, dtype=rhs.dtype)
            k_base = dynamics.linear.solve_base(p, tau, transpose=transpose)
            k = k_base.T
            gram = p @ k
            eye = torch.eye(gram.shape[0], device=rhs.device, dtype=rhs.dtype)
        ...
        y = dynamics.linear.solve_base(rhs_flat, tau, transpose=transpose).reshape(sample_count, direction_count, latent_dim)
        small = eye.unsqueeze(0) - float(tau) * torch.bmm(
            reduced,
            gram.expand(sample_count, -1, -1),
        )
        py = y.reshape(sample_count * direction_count, latent_dim).matmul(p.T).reshape(sample_count, direction_count, -1)
        reduced_rhs = torch.bmm(reduced, py.transpose(1, 2)).transpose(1, 2)
        eta = torch.linalg.solve(small, reduced_rhs.transpose(1, 2)).transpose(1, 2)
        correction = eta.reshape(sample_count * direction_count, -1).matmul(k.T).reshape(sample_count, direction_count, latent_dim)
...
def _linear_vjp_batched(
    ...
    *,
    ...
    if include_x_grad:
        ...
        x_grad = None
    if isinstance(linear, DenseLinearA):
        _add_param_grad(param_grads, "linear.A", torch.matmul(cot.permute(1, 2, 0), x))
        return x_grad
    if isinstance(linear, DissipativeSkewA):
        diagonal_cot = (cot * x[:, None, :]).sum(dim=0)
        ...
        if linear.skew_rank > 0:
            ...
            xq = x @ q
            xp = x @ p
            cot_flat = cot.reshape(sample_count * direction_count, latent_dim)
            cotq = cot_flat.matmul(q).reshape(sample_count, direction_count, -1)
            cotp = cot_flat.matmul(p).reshape(sample_count, direction_count, -1)
            grad_p = torch.matmul(cot.permute(1, 2, 0), xq) - torch.matmul(x.T.unsqueeze(0), cotq.permute(1, 0, 2))
            grad_q = torch.matmul(x.T.unsqueeze(0), cotp.permute(1, 0, 2)) - torch.matmul(cot.permute(1, 2, 0), xp)
            _add_param_grad(param_grads, "linear.P", grad_p)
            _add_param_grad(param_grads, "linear.Q", grad_q)
        return x_grad
    raise TypeError(f"unsupported linear type {type(linear)!r}")
...
def _source_vjp_batched(
    ...
    *,
    ...
    if not isinstance(dynamics.source, LinearSource):
        return None
    _add_param_grad(param_grads, "source.B", float(scale) * torch.matmul(cot.permute(1, 2, 0), p_mid))
    if dynamics.source.c is not None:
        _add_param_grad(param_grads, "source.c", float(scale) * cot.sum(dim=0))
    if not return_input_grad:
        return None
    b = dynamics.source.B.to(device=cot.device, dtype=cot.dtype)
    return float(scale) * cot.reshape(sample_count * direction_count, latent_dim).matmul(b).reshape(sample_count, direction_count, -1)
...
def _energy_tuckertt_frozen_vjp_batched(
    ...
    *,
    ...
    sample_count, direction_count, latent_dim = cot.shape
    q = int(p.shape[0])
    z_outer = torch.bmm(z_lag.unsqueeze(2), z.unsqueeze(1)).reshape(sample_count, q * q)
    reduced_out = z_outer.matmul(c.reshape(q, q * q).T)
    omega = cot.reshape(sample_count * direction_count, latent_dim).matmul(p.T).reshape(sample_count, direction_count, q)

    omega_rows = omega.permute(1, 2, 0).reshape(direction_count * q, sample_count)
    grad_c = omega_rows.matmul(z_outer).reshape(direction_count, q, q, q)

    c_for_lag = c.reshape(q * q, q)
    c_z = z.matmul(c_for_lag.T).reshape(sample_count, q, q)
    grad_z_lag = torch.bmm(omega, c_z)

    c_for_x = c.permute(0, 2, 1).contiguous().reshape(q * q, q)
    c_z_lag = z_lag.matmul(c_for_x.T).reshape(sample_count, q, q)
    grad_z = torch.bmm(omega, c_z_lag)

    ell_grad = grad_z_lag.reshape(sample_count * direction_count, q).matmul(p).reshape(sample_count, direction_count, latent_dim)
    x_grad = (
        grad_z.reshape(sample_count * direction_count, q).matmul(p).reshape(sample_count, direction_count, latent_dim)
        if include_x_grad
        else None
    )

    grad_basis = cot.permute(1, 2, 0).reshape(direction_count * latent_dim, sample_count).matmul(reduced_out)
    grad_basis = grad_basis.reshape(direction_count, latent_dim, q).transpose(1, 2).contiguous()
    grad_basis = grad_basis + grad_z_lag.permute(1, 2, 0).reshape(direction_count * q, sample_count).matmul(ell).reshape(
        direction_count,
        q,
        latent_dim,
    )
    grad_basis = grad_basis + grad_z.permute(1, 2, 0).reshape(direction_count * q, sample_count).matmul(x).reshape(
        direction_count,
        q,
        latent_dim,
    )
...
def lagged_midpoint_rollout_adjoint_batched(
    ...
    *,
    ...
    dense_a = dynamics.dense_A().to(device=states_tensor.device, dtype=states_tensor.dtype)
    tau = 0.5 * float(h)
    basis = dynamics.quadratic.basis.to(device=states_tensor.device, dtype=states_tensor.dtype)
    k_base = dynamics.linear.solve_base(basis, tau, transpose=True)
    k_t = k_base.T
    gram = basis @ k_t
    transpose_solve_cache = BatchedEnergyTuckerSolveCache(
        basis=basis,
        k_base=k_base,
        k_t=k_t,
        gram=gram,
        eye=torch.eye(gram.shape[0], device=states_tensor.device, dtype=states_tensor.dtype),
        tau=float(tau),
        transpose=True,
    )
    ...
    with _quadratic_cache_context(dynamics):
        for n in reversed(range(steps)):
            ...
            lambda_u, lambda_p, step_grads = _lagged_midpoint_step_adjoint_manual_batched(
                ...
                dense_a=dense_a,
                energy_accumulator=energy_accumulator,
                solve_cache=transpose_solve_cache,
            )
