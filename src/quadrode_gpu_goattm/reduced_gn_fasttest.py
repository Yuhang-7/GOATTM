...
def _linear_param_action_batched(
    ...
    if isinstance(linear, DenseLinearA):
        ...
        return torch.einsum("nb,kab->nka", x, d_a)
    if isinstance(linear, DissipativeSkewA):
        ...
        if linear.skew_rank > 0:
            ...
            out = (
                out
                + torch.einsum("nks,rs->nkr", torch.einsum("nr,krs->nks", x, dq), p)
                + torch.einsum("ns,krs->nkr", x @ q, dp)
                - torch.einsum("nks,rs->nkr", torch.einsum("nr,krs->nks", x, dp), q)
                - torch.einsum("ns,krs->nkr", x @ p, dq)
            )
        return out
...
def _source_param_action_batched(
    ...
    if p_mid is not None:
        ...
        out = out + torch.einsum("ni,kri->nkr", p_mid, d_b)
...
def _reconstruct_energy_tucker_tensor_dot_batched(
    ...
    *,
    ...
    free_full_dot = (
        torch.einsum("kar,rbs,sc->kabc", dc0, c1, c2)
        + torch.einsum("ar,krbs,sc->kabc", c0, dc1, c2)
        + torch.einsum("ar,rbs,ksc->kabc", c0, c1, dc2)
    )
...
def _energy_tucker_left_apply(c: torch.Tensor, dz_ell: torch.Tensor, z_x: torch.Tensor) -> torch.Tensor:
    sample_mats = torch.einsum("abc,nc->nab", c, z_x)
...
def _energy_tucker_right_apply(c: torch.Tensor, z_ell: torch.Tensor, dz_x: torch.Tensor) -> torch.Tensor:
    sample_mats = torch.einsum("abc,nb->nac", c, z_ell)
...
def _energy_tucker_param_core_apply(dc: torch.Tensor, z_ell: torch.Tensor, z_x: torch.Tensor) -> torch.Tensor:
    n = z_ell.shape[0]
    k, q, _, _ = dc.shape
    z_outer = torch.bmm(z_ell[:, :, None], z_x[:, None, :]).reshape(n, q * q)
    return torch.matmul(dc.reshape(k, q, q * q), z_outer.T).permute(2, 0, 1).contiguous()
...
def _quadratic_frozen_param_action_batched(
    ...
    reduced_dot = (
        _energy_tucker_param_core_apply(dc, z_lag, z)
...
def _energy_tucker_frozen_tangent_batched(
    ...
    *,
    ...
    reduced_dot = reduced_dot + _energy_tucker_param_core_apply(dc, z_ell, z_x)
    if reduced_matrix is None:
        reduced = torch.einsum("abc,nb,nc->na", c, z_ell, z_x)
    else:
    ...
    *,
    ...
    with torch.no_grad(), _quadratic_cache_context(dynamics):
        for n in range(steps):
            ...
            y_dot = u_dot + float(h) * _rhs_tangent_batched(dynamics, u, u_dot, p_n, directions, quad_cache=quad_cache)
            iterates_dot = [y_dot]
            ...
            for j in range(picard_iterates.shape[0] - 1):
                ...
                rhs_dot = (
                    ...
                    + tau * _quadratic_frozen_tangent_batched(dynamics, ell, u, ell_dot, u_dot, directions, quad_cache=quad_cache)
                    + float(h)
                    * _source_param_action_batched(
                        dynamics,
                        p_n,
                        u_dot,
                        directions,
                ...
                y_dot = _exact_frozen_solve_batched(dynamics, ell, rhs_dot, tau, quad_cache=quad_cache)
...
def projected_decoder_residual_tangent_batched(
    ...
    *,
    ...
    for start in range(0, leading, max(1, int(chunk_size))):
        ...
        d_normal.add_(torch.einsum("mkf,mg->kfg", dfeat, weighted_features))
        d_normal.add_(torch.einsum("mf,mkg->kfg", features, weighted_dfeat))
        d_rhs.add_(torch.einsum("mo,mkf->kof", flat_targets[start:end], weighted_dfeat))
    ...
    rhs_for_dcoeff_t = d_rhs.transpose(1, 2) - torch.einsum("kfg,og->kfo", d_normal, coeff)
    ...
    for start in range(0, leading, max(1, int(chunk_size))):
        ...
        features = decoder.features(u)
        dfeat = decoder_feature_tangent_chunk_batched(decoder, u, flat_dot[start:end])
        dres = torch.einsum("mkf,of->mko", dfeat, coeff_no_bias) + torch.einsum("mf,kof->mko", features, dcoeff_no_bias)
...
def _masked_cross_output_cotangent_state_grad_batched(
    ...
    *,
    ...
    grad = torch.einsum("mko,or->mkr", output_cotangent, coeff_no_bias[:, : decoder.latent_dim])
    if not getattr(decoder, "include_quadratic", False):
        return grad
    if not hasattr(decoder, "quadratic_i"):
        feature_cotangent = torch.einsum("mko,of->mkf", output_cotangent, coeff_no_bias)
    ...
    for start in range(0, idx_i_all.numel(), chunk):
        ...
        quad_cot = torch.einsum("mko,oq->mkq", output_cotangent, quad_coeff[:, start:end])
...
def _gauss_newton_state_cotangents_batched_masked_cross_direct(
    ...
    *,
    ...
    for start in range(0, leading, row_chunk):
        ...
        coeff_cross = torch.einsum("mko,mf->kof", weighted_d_pred, features_aug)
    ...
    for start in range(0, leading, row_chunk):
        ...
        dres = d_pred_fixed + torch.einsum("mf,kof->mko", features_aug, dcoeff)
        if flat_weights is None:
        ...
        state_cot[start:end] = _masked_cross_output_cotangent_state_grad_batched(
            decoder,
            u,
            weighted_output,
            coeff_no_bias,
    ...
    *,
    ...
    for start in range(0, leading, chunk_size):
        ...
        if flat_weights is None:
            ...
            qform = qform + (dres.square().sum(dim=2) * flat_weights[start:end, None]).sum(dim=0)
        feature_cot = torch.einsum("mko,of->mkf", weighted, coeff_no_bias)
...
def _decoder_tangent_normal_terms_for_state(
    ...
    *,
    ...
    if weights is None:
        ...
        weighted_features = features * w[:, None]
        weighted_dfeat = dfeat * w[:, None, None]
    d_normal = torch.einsum("mkf,mg->kfg", dfeat, weighted_features)
    d_normal = d_normal + torch.einsum("mf,mkg->kfg", features, weighted_dfeat)
    d_rhs = torch.einsum("mo,mkf->kof", targets, weighted_dfeat)
    ...
    for j in range(picard_iterates.shape[0] - 1):
        ...
        if isinstance(dynamics.quadratic, EnergyTuckerTTQuadratic):
            if quad_cache is None:
                ...
                c = quad_cache.reduced_tensor
            z_ell = ell @ p.T
            reduced_matrix = torch.einsum("abc,nb->nac", c, z_ell)
        rhs_dot = (
            u_dot
            + tau * linear_tangent
            + tau
...
def _masked_cross_prediction_tangent_batched(
    ...
    *,
    ...
    d_pred = torch.einsum("mkr,or->mko", states_dot, coeff_no_bias[:, : decoder.latent_dim])
    if not getattr(decoder, "include_quadratic", False):
        return d_pred
    if not hasattr(decoder, "quadratic_i"):
        dfeat = decoder_feature_tangent_batched(decoder, states, states_dot, chunk_size=states.shape[0])
        return torch.einsum("mkf,of->mko", dfeat, coeff_no_bias)

    idx_i = decoder.quadratic_i.to(device=states.device)
    idx_j = decoder.quadratic_j.to(device=states.device)
    quad_coeff = coeff_no_bias[:, decoder.latent_dim :]
    output_dim = int(quad_coeff.shape[0])
    chunk = max(1, int(output_chunk_size))
    scatter_i = idx_i.reshape(1, 1, -1).expand(states.shape[0], min(chunk, output_dim), -1)
    scatter_j = idx_j.reshape(1, 1, -1).expand(states.shape[0], min(chunk, output_dim), -1)
    for start in range(0, output_dim, chunk):
        end = min(start + chunk, output_dim)
        coeff_chunk = quad_coeff[start:end]
        grad = states.new_zeros(states.shape[0], end - start, decoder.latent_dim)
        if scatter_i.shape[1] != end - start:
            scatter_i_chunk = idx_i.reshape(1, 1, -1).expand(states.shape[0], end - start, -1)
            scatter_j_chunk = idx_j.reshape(1, 1, -1).expand(states.shape[0], end - start, -1)
        else:
            scatter_i_chunk = scatter_i
            scatter_j_chunk = scatter_j
        values_i = states.index_select(1, idx_j)[:, None, :] * coeff_chunk[None, :, :]
        values_j = states.index_select(1, idx_i)[:, None, :] * coeff_chunk[None, :, :]
        grad.scatter_add_(2, scatter_i_chunk, values_i)
        grad.scatter_add_(2, scatter_j_chunk, values_j)
        d_pred[:, :, start:end].add_(torch.einsum("mkr,mor->mko", states_dot, grad))
    return d_pred
...
def _add_masked_cross_tangent_residual_terms_(
    ...
    *,
    ...
    rhs_for_dcoeff_t[:, : decoder.latent_dim, :].add_(-torch.einsum("mkr,mo->kro", states_dot, weighted_residual))
    if not getattr(decoder, "include_quadratic", False):
        return
    if not hasattr(decoder, "quadratic_i"):
        dfeat = decoder_feature_tangent_batched(decoder, states, states_dot, chunk_size=states.shape[0])
        rhs_for_dcoeff_t[:, decoder.latent_dim :, :].add_(
            -torch.einsum("mkf,mo->kfo", dfeat[:, :, decoder.latent_dim :], weighted_residual)
        )
        return
    ...
    for start in range(0, idx_i_all.numel(), chunk):
        ...
        rhs_for_dcoeff_t[:, feature_offset + start : feature_offset + end, :].add_(
            -torch.einsum("mkq,mo->kqo", quad_dot, weighted_residual)
        )
...
def _single_pass_schur_sketch_state_(
    ...
    *,
    ...
    coeff_no_bias = coeff[:, : decoder.feature_dim]
    d_pred_fixed = torch.einsum("mkr,or->mko", states_dot, coeff_no_bias[:, : decoder.latent_dim])
    if weights is None:
        weighted_residual = residual
        sqrt_weight = None
    else:
    ...
    if getattr(decoder, "include_quadratic", False):
        if not hasattr(decoder, "quadratic_i"):
            dfeat = decoder_feature_tangent_batched(decoder, states, states_dot, chunk_size=states.shape[0])
            d_pred_fixed.add_(torch.einsum("mkf,of->mko", dfeat[:, :, decoder.latent_dim :], coeff_no_bias[:, decoder.latent_dim :]))
            rhs_for_dcoeff_t[:, decoder.latent_dim :, :].add_(
                -torch.einsum("mkf,mo->kfo", dfeat[:, :, decoder.latent_dim :], weighted_residual)
            )
        else:
            ...
            for start in range(0, idx_i_all.numel(), chunk):
                ...
                d_pred_fixed.add_(torch.einsum("mkq,oq->mko", quad_dot, coeff_chunk))
                rhs_for_dcoeff_t[:, feature_offset + start : feature_offset + end, :].add_(
                    -torch.einsum("mkq,mo->kqo", quad_dot, weighted_residual)
                )
    if weights is None:
        ...
        if fixed_fixed_gram is not None:
            fixed_fixed_gram.add_(torch.einsum("mko,mlo->kl", d_pred_fixed, d_pred_fixed))
    else:
        weighted_d_pred = d_pred_fixed * w[:, None, None]
        qform = (d_pred_fixed.square().sum(dim=2) * w[:, None]).sum(dim=0)
        if fixed_fixed_gram is not None:
            fixed_fixed_gram.add_(torch.einsum("mko,mlo,m->kl", d_pred_fixed, d_pred_fixed, w))
    coeff_cross = torch.einsum("mko,mf->kof", weighted_d_pred, features)
    rhs_for_dcoeff_t.add_(-coeff_cross.transpose(1, 2))
    ...
    def sketched_jvp_batch_from_flat_single_pass(
        ...
        *,
        ...
        with torch.no_grad(), _quadratic_cache_context(self.dynamics):
            ...
            dcoeff = solved.reshape(feature_dim, direction_count, output_dim).permute(1, 2, 0).contiguous()
            coeff_residual_sketch = torch.einsum("sf,kof->sko", feature_sketch, dcoeff)
            ...
            fixed_coeff_gram = torch.einsum("kof,lof->kl", fixed_coeff_cross, dcoeff)
            coeff_gram = torch.einsum("kof,fg,log->kl", dcoeff, normal_no_ridge, dcoeff)
            exact_projected_gram = fixed_fixed_gram + fixed_coeff_gram + fixed_coeff_gram.T + coeff_gram
            dcoeff_qform = 2.0 * torch.einsum("kof,kof->k", fixed_coeff_cross, dcoeff)
            dcoeff_qform = dcoeff_qform + torch.einsum("kof,fg,kog->k", dcoeff, normal_no_ridge, dcoeff)
            qform = fixed_qform + dcoeff_qform
            if float(self.objective.decoder_ridge) > 0.0:
                ridge_rows = float(self.objective.decoder_ridge) ** 0.5 * dcoeff.reshape(direction_count, -1).T
                sketch_matrix = torch.cat((sketch_matrix, ridge_rows), dim=0)
                ridge_gram = float(self.objective.decoder_ridge) * torch.einsum("kof,lof->kl", dcoeff, dcoeff)
