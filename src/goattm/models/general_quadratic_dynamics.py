...
from goattm.core.parametrization import compressed_quadratic_dimension
...
class GeneralQuadraticDynamics:
    a: np.ndarray
    h_matrix: np.ndarray
    c: np.ndarray
    ...
    def __post_init__(self) -> None:
        ...
        r = self.a.shape[0]
        expected_h_shape = (r, compressed_quadratic_dimension(r))
        if self.h_matrix.ndim != 2 or self.h_matrix.shape != expected_h_shape:
            raise ValueError(f"h_matrix must have shape {expected_h_shape}, got {self.h_matrix.shape}")
